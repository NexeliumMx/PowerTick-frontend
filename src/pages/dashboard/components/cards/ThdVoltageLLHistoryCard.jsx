...
const ThdVoltageLLHistoryCard = ({ selectedPowerMeter, measurementRange, defaultTimeFilter, t: tProp }) => {
  ...
  const { thdVoltageLLHistory } = useApiData();
  // Use hook
  const { data: thdVoltageLLHistoryData, isLoading } = thdVoltageLLHistory(
    user_id,
    selectedPowerMeter,
    start_utc,
    end_utc,
  ...
  const chartData = thdVoltageLLHistoryData?.map((item) => ({
    name: dayjs.utc(item.utc_time).tz(tz).format('HH:mm'),
    timestamp_local: dayjs.utc(item.utc_time).tz(tz).format('YYYY-MM-DD HH:mm'),
    thdVoltageAB: item.thd_voltage_ab,
    thdVoltageBC: item.thd_voltage_bc,
    thdVoltageCA: item.thd_voltage_ca,
  }));
  ...
  const cardTitle = t('Analysis.thdVoltageLLHistory', 'THD Voltage LL History');
    ...
    <Card sx={{ minHeight: "580px", display: "flex", flexDirection: "column", backgroundColor: theme.palette.background.card }}>
      ...
      <CardContent sx={{ flexGrow: 1, pt: 0 }}>
        <Box sx={{ width: "100%", overflow: "auto", px: 2, my: -5 }}>
          {isLoading ? (
            <ChartSkeletonCard />
          ) : thdVoltageLLHistoryData && thdVoltageLLHistoryData.length > 0 ? (
            <ResponsiveContainer width="100%" height={400}>
              <LineChart data={chartData} margin={{ top: 100, right: 30, left: 40, bottom: 60 }}>
                ...
                <Line 
                  type="monotone" 
                  dataKey="thdVoltageAB" 
                  stroke={chartColors.voltageAB || "#ff7300"} 
                  strokeWidth={2}
                  name="THD Voltage AB"
                  dot={{ fill: chartColors.voltageAB || "#ff7300", strokeWidth: 1, r: 3 }}
                />
                <Line 
                  type="monotone" 
                  dataKey="thdVoltageBC" 
                  stroke={chartColors.voltageBC || "#8884d8"} 
                  strokeWidth={2}
                  name="THD Voltage BC"
                  dot={{ fill: chartColors.voltageBC || "#8884d8", strokeWidth: 1, r: 3 }}
                />
                <Line 
                  type="monotone" 
                  dataKey="thdVoltageCA" 
                  stroke={chartColors.voltageCA || "#82ca9d"} 
                  strokeWidth={2}
                  name="THD Voltage CA"
                  dot={{ fill: chartColors.voltageCA || "#82ca9d", strokeWidth: 1, r: 3 }}
                />
              </LineChart>
            </ResponsiveContainer>
          ) : (
...
ThdVoltageLLHistoryCard.propTypes = {
...
export default ThdVoltageLLHistoryCard;
